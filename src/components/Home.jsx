    ...
    <div>
      ...
      <section className="bg-gradient-to-r from-orange-400 to-amber-600 text-white text-center pt-10 pb-16 space-y-5">
        <h1 className="text-2xl sm:text-5xl font-semibold sm:font-bold">
          Welcome to <span className="text-primary">ShoppyGlobe</span>
        </h1>
        ...
        <div className="text-center space-x-6 pt-5">
          <Link to={"/products"} className="bg-white text-orange-500 border-2 border-white font-semibold py-3 px-6 rounded-xl cursor-pointer hover:scale-105 transition-transform">
            Shop Now
          </Link>
          <Link to={"/learn-more"} className="text-white border-2 font-semibold py-3 px-6 rounded-xl hover:bg-white hover:text-orange-500 hover:border-white transition-colors duration-300 cursor-pointer">
            Learn More
          </Link>
        </div>
      </section>
        ...
        <div className="flex items-center justify-between py-5">
          <div>
            ...
            <p className="text-gray-600">
              {(search.trim() ? filteredProducts : popularProducts).length} products found
            </p>
          </div>
          <Link to={"/products"} className="flex items-center gap-4 text-blue-500">
            <span className="font-semibold">View all</span> <MoveRight />
          </Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pb-10">
          {(search.trim() ? filteredProducts : popularProducts).map((item) => (
